...
def salva_db_pazienti(registro):
    """Salva l'intero dizionario dei pazienti aggiornando Google Sheets."""
    try:
        conn = st.connection("gsheets", type=GSheetsConnection)
        righe = []
        ...
        print(f"Errore nel salvataggio completo del DB su Google Sheets: {e}")
        return False
...
def genera_o_aggiorna_registro(nome, cognome, data_nascita, codice_univoco):
    """Salva o aggiorna il singolo paziente nel Foglio Google."""
    try:
        conn = st.connection("gsheets", type=GSheetsConnection)
        df = conn.read(ttl=0)
        ...
        if df.empty or "codice" not in df.columns:
            ...
            # Se esiste già, aggiorniamo solo i dati anagrafici di base preservando il resto se possibile, o rimpiazzando la riga
...
def render_anamnesi_generale(prefix="gen"):
    ...
    with col_v1:
        scala_performance = st.selectbox(
            ...
            [
                ...
                (
                    "3 - Capace di limitata cura personale, confinato a letto/sedia"
                    " (>50% del giorno)"
                ),
    ...
    with col_vac1:
        vac_antinfluenzale = st.checkbox(
            "Antinfluenzale (Ultima stagione)", key=f"{prefix}_vac_influenzale"
        )
    ...
    organi_fam = []
    if fam_prostata:
        organi_fam.append("Prostata")
    if fam_seno:
        organi_fam.append("Seno")
    if fam_utero:
        organi_fam.append("Utero")
    if fam_ovaio:
        organi_fam.append("Ovaio")
    if fam_vescica:
        organi_fam.append("Vescica")
    if fam_colon:
        organi_fam.append("Colon-Retto")
    if fam_altro:
        organi_fam.append("Altro/Negativa")
    str_fam = ", ".join(organi_fam) if organi_fam else ""

    vaccini = []
    if vac_antinfluenzale:
        vaccini.append("Antinfluenzale")
    if vac_pneumococco:
        vaccini.append("Anti-Pneumococco")
    if vac_zoster:
        vaccini.append("Anti-Herpes Zoster")
    if vac_covid:
        vaccini.append("Anti-COVID")
...
def formatta_anamnesi_per_pdf(anamnesi):
    ...
    if anamnesi.get("insuff_renale"):
        ...
        patologie.append(
            ...
            + (f" (Creatinina: {crea} mg/dL)" if crea else "")
        )
    ...
    if peso > 0 and altezza > 0:
        linee.append(
            f"• Parametri Antropometrici: Peso {peso} kg, Altezza {altezza} cm, BMI"
            f" {bmi} kg/m²"
        )
...
def genera_pdf_referto(
    ...
    story.append(
        Paragraph(
            "<b><font color='#CC0000'>2</font><font"
            " color='#1A1A1A'>gether</font></b>",
            style_brand_title,
        )
    )
    story.append(
        Paragraph("the answer is just next 2U", style_brand_sub)
    )
