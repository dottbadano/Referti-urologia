...
from utils import salva_db_pazienti, genera_o_aggiorna_registro, genera_pdf_referto, ELENCO_MESI
...
def calcola_gruppo_rischio_eau(isup_num, psa, ct_stage, gleason_terziario):
    ...
    # 1. Controllo prioritario e assoluto per cT3a/b/c, ISUP elevati, PSA > 20 o pattern terziario aggressivo
    ...
    # 2. Rischio intermedio (favorevole o sfavorevole), escludendo categoricamente i cT3a
...
def render_modulo():
    ...
    if modalita == "1. Prima Visita: Inquadramento Bioptico & Rischio":
        ...
        if st.button("💾 Salvataggio & Genera Report PDF (Prostata)"):
        ...
        if codice_search in st.session_state["db_pazienti"]:
            ...
            if st.button("💾 Salvataggio Visita & Genera PDF Controllo"):
